...
uint8_t vulkan_renderer_initialize(void* vulkan_renderer_state, const char* application_name, uint32_t width, uint32_t height) {
    ...
    // 1000 KiloBytes... should be enough
    temp->requested_layer_extensions = malloc(1000);

    temp->requested_layer_extensions[0] = "VK_KHR_surface";
    
#ifdef S_LINUX_
    temp->requested_layer_extensions[2] = "VK_KHR_xcb_surface";
#elif S_WINDOWS_
    temp->requested_layer_extensions[2] = "VK_KHR_win32_surface";
#endif

#ifdef _DEBUG
    temp->requested_layer_extension_count = 3;
    temp->requested_layer_extensions[1] = "VK_EXT_debug_utils";
#else
    temp->requested_layer_extension_count = 2;
#endif
